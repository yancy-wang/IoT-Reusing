...
voc_pdf = "/Users/wangyangyang/Desktop/Finland/summer_job/ubikampus/thunderboard_voc_only.pdf"
...
with PdfPages(output_pdf) as pdf:
    for sensor_type in sensor_types:
        ...
        for device_id, files in device_data.items():
            ...
            filtered_wio_df = combined_df[(combined_df['Timestamp'] >= start_time) & (filtered_wio_df['Timestamp'] <= end_time)]
...
# 单独绘制 Thunderboard Sense 2 的 VOC 数据图表
with PdfPages(voc_pdf) as pdf_voc:
    plt.figure(figsize=(10, 6))

    # 只绘制 VOC 的平滑数据
    for device, group in thunderboard_df.groupby('Device'):
        smoothed_voc_data = smooth_data(group['VOCs'])  # 对 VOC 数据进行平滑处理
        plt.plot(group['Timestamp'], smoothed_voc_data, label=device, color=color_map[device])

    plt.xlabel("Time")
    plt.ylabel("VOCs")
    plt.title("VOC Data for Thunderboard Sense 2 (Smoothed)")
    plt.legend()
    plt.xticks(rotation=45)
    plt.tight_layout()
    pdf_voc.savefig()
...
print(f"All plots saved to {output_pdf} and VOC plot saved to {voc_pdf}.")
