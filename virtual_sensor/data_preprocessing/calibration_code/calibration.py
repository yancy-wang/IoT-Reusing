...
# 找到基准传感器（相关性中位数传感器）
def find_reference_device(correlation_matrix):
    median_correlation = correlation_matrix.median(axis=1)
...
with PdfPages(output_pdf) as pdf:
    ...
    for sensor_type in sensor_types:
        ...
        # 选择基准传感器
        reference_device = find_reference_device(correlation_matrix)
        # 对数据进行校准
