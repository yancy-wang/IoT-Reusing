...
# Mapping for sensor type renaming
sensor_rename_map = {
...
def plot_comparison(original_device_data, adjusted_device_data, sensor_types):
    ordered_sensors = ['Sensor 1', 'Sensor 2', 'Sensor 3', 'Sensor 4', 'Sensor 5', 'Sensor 7','Sensor 8']
        ...
        for sensor_type in sensor_types:
            ...
            for sensor in ordered_sensors:
                ...
                if device_mac and device_mac[0] in original_device_data:
                    ...
                    # Multiply CO2 data by 100 if needed
                    if sensor_type == 'CO2(ppm)':
                            y_data = y_data * 100

                    # Get the new label for the sensor type
                    sensor_label = sensor_rename_map.get(sensor_type, sensor_type)

                    axs[0].plot(x_data, y_data, label=sensor, color=color_map[sensor])

            axs[0].set_xlabel("Time")
            axs[0].set_ylabel(sensor_label)
            axs[0].set_title(f"{sensor_label} Data Comparison (Original)")
            axs[0].legend()
            ...
            axs[1].set_ylabel(sensor_label)
            axs[1].set_title(f"{sensor_label} Data Comparison (Adjusted)")
            axs[1].legend()
