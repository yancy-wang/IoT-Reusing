...
def plot_adjusted_data(sensor_type, combined_data, adjusted_data, output_folder):
    for mac in adjusted_data.columns:
        ...
        plt.plot(combined_data[mac]['Timestamp'], combined_data[mac][sensor_type], label=f'Original {mac}')
        plt.plot(adjusted_data['Timestamp'], adjusted_data[mac], label=f'Adjusted {mac}')
        plt.xlabel('Timestamp')
        plt.ylabel(sensor_type)
        plt.title(f'Adjusted {sensor_type} Data for {mac}')
        plt.legend()
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(os.path.join(output_folder, f'{sensor_type}_adjusted_{mac}.png'))
        plt.close()
...
for sensor_type in sensor_types:
    ...
    adjusted_df = adjust_data(high_corr_df.drop(columns=['Timestamp']).copy(), reference_mac)
    ...
    # 绘制调整后的数据图像
    plot_adjusted_data(sensor_type, combined_data, adjusted_df, output_folder)
