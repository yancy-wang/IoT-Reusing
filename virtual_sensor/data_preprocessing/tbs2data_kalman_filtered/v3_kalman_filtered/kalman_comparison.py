...
original_csv_directory = "/Users/wangyangyang/Desktop/Finland/summer_job/data_all/tbs2data_12/tbs2data_12_v3"
input_folder = "/Users/wangyangyang/Desktop/Finland/summer_job/data_all/tbs2data_kalman_filtered/v3_kalman_filtered"
output_pdf = "/Users/wangyangyang/Desktop/Finland/summer_job/data_all/tbs2data_12/tbs2data_12_v3/sensor_data_comparison_combined.pdf"
adjusted_csv_folder = "/Users/wangyangyang/Desktop/Finland/summer_job/data_all/tbs2data_kalman_filtered/v3_calibration_intercept"
intercept_file_path = "/Users/wangyangyang/Desktop/Finland/summer_job/data_all/tbs2data_kalman_filtered/v3_calibration_intercept/adjustments.csv"

# MAC address to sensor number mapping
mac_to_sensor = {
    '90:FD:9F:7B:83:D6': 'Sensor 3',
    '90:FD:9F:7B:81:06': 'Sensor 4',
    '90:FD:9F:7B:83:D1': 'Sensor 9',
    '90:FD:9F:7B:84:67': 'Sensor 10',
    '00:0D:6F:20:D3:F3': 'Sensor 11',
    '90:FD:9F:7B:82:50': 'Sensor 13',
    '00:0D:6F:20:D4:96': 'Sensor 14'   
}

# Define color mapping for sensors
color_map = {
    'Sensor 3': '#bcbd22',  # Yellow-green
    'Sensor 4': '#d62728',  # Red
    'Sensor 9': '#e377c2',  # Pink
    'Sensor 10': '#17becf',  # Cyan
    'Sensor 11': '#1f77b4',  # Blue
    'Sensor 13': '#ff7f0e',  # Orange
    'Sensor 14': '#8c564b'  # Brown  
...
# Function to generate adjusted plots
def generate_adjusted_plots(sensor_types):
    ...
    for sensor_type in sensor_types:
        ...
        adjusted_device_data[sensor_type] = adjusted_df
        for mac, adjustment in adjustments.items():
    ...
    return adjusted_device_data

# Plot original and adjusted data together
def plot_comparison(original_device_data, adjusted_device_data, sensor_types):
    ordered_sensors = ['Sensor 3', 'Sensor 4', 'Sensor 9', 'Sensor 10', 'Sensor 11', 'Sensor 13', 'Sensor 14']
    with PdfPages(output_pdf) as pdf:
        for sensor_type in sensor_types:
            if sensor_type not in adjusted_device_data:
                continue
            fig, axs = plt.subplots(2, 1, figsize=(10, 12))

            # Plot original data
            for sensor in ordered_sensors:
                device_mac = [mac for mac, sensor_id in mac_to_sensor.items() if sensor_id == sensor]
                if device_mac and device_mac[0] in original_device_data:
                    combined_df = pd.concat([pd.read_csv(f) for f in original_device_data[device_mac[0]]])
                    combined_df['Timestamp'] = pd.to_datetime(combined_df['Timestamp'])
                    combined_df.sort_values('Timestamp', inplace=True)
                    x_data = combined_df["Timestamp"]
                    y_data = combined_df[sensor_type]

                    axs[0].plot(x_data, y_data, label=sensor, color=color_map[sensor])

            axs[0].set_xlabel("Time")
            axs[0].set_ylabel(sensor_type)
            axs[0].set_title(f"{sensor_type} Data Comparison (Original)")
            axs[0].legend()
            axs[0].tick_params(axis='x', rotation=45)

            # Plot adjusted data
            adjusted_df = adjusted_device_data[sensor_type]
            for sensor in ordered_sensors:
                if sensor in adjusted_df.columns:
                    x_data = adjusted_df["Timestamp"]
                    y_data = adjusted_df[sensor]

                    axs[1].plot(x_data, y_data, label=sensor, color=color_map[sensor])

            axs[1].set_xlabel("Time")
            axs[1].set_ylabel(sensor_type)
            axs[1].set_title(f"{sensor_type} Data Comparison (Adjusted)")
            axs[1].legend()
...
def main():
    ...
    adjusted_device_data = generate_adjusted_plots(sensor_types)
