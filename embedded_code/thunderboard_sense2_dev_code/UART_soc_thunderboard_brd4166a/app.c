...
static app_timer_t app_periodic_test_timer;
...
static void app_periodic_test_timer_cb(app_timer_t *timer, void *data);

void exp_printf(const char *format, ...)
{
  char buf[128];
  ...
  sl_iostream_write(sl_iostream_exp_handle, buf, strlen(buf));
  va_end(args);
}
...
void app_init(void)
{
  ...
  sensor_init();
  sc = app_timer_start(&app_periodic_test_timer,
                          1000,
                          app_periodic_test_timer_cb,
                          NULL,
                          true);
}
...
static void app_periodic_test_timer_cb(app_timer_t *timer, void *data)
{
    ...
    sl_gatt_service_light_get(&lux, &uvi);
    sl_gatt_service_pressure_get(&pressure);
    sl_gatt_service_gas_get(&eco2, &tvoc);
    sl_gatt_service_sound_get(&sound_level);
