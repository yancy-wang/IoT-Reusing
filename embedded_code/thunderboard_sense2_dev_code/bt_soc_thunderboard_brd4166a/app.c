...
SL_WEAK void app_process_action(void)
{
  ...
  float field_strength = 0.0;
    ...
    int16_t ovec[3] = {0, 0, 0};
    int16_t avec[3] = {0, 0, 0};
  ...
  #ifdef SL_CATALOG_SENSOR_HALL_PRESENT
    if (sl_sensor_hall_get(&field_strength, &alert, &tamper) == SL_STATUS_OK) {
      app_log_info("Magnetic flux = %4.3f mT" APP_LOG_NL, field_strength);
    }
  #endif

  #ifdef SL_CATALOG_SENSOR_LIGHT_PRESENT
    if (sl_sensor_light_get(&lux, &uvi) == SL_STATUS_OK) {
  ...
  #ifdef SL_CATALOG_SENSOR_RHT_PRESENT
    if (sl_sensor_rht_get(&humidity, &temperature) == SL_STATUS_OK) {
      app_log_info("Humidity = %3.2f %%RH" APP_LOG_NL, (float)humidity / 1000.0f);
      app_log_info("Temperature = %3.2f C" APP_LOG_NL, (float)temperature / 1000.0f);
    }
  #endif

  #ifdef SL_CATALOG_SENSOR_IMU_PRESENT
    if (sl_sensor_imu_get(ovec, avec) == SL_STATUS_OK) {
  ...
  #ifdef SL_CATALOG_SENSOR_PRESSURE_PRESENT
    if (sl_sensor_pressure_get(&pressure) == SL_STATUS_OK) {
      app_log_info("Pressure = %0.3f mbar" APP_LOG_NL, pressure);
    }
  #endif

  #ifdef SL_CATALOG_SENSOR_GAS_PRESENT
    if (sl_sensor_gas_get(&eco2, &tvoc) == SL_STATUS_OK) {
      app_log_info("eCO2 = %u ppm" APP_LOG_NL, eco2);
      app_log_info("TVOC = %u ppd" APP_LOG_NL, tvoc);
    }
  #endif

  #ifdef SL_CATALOG_SENSOR_SOUND_PRESENT
    if (sl_sensor_sound_get(&sound_level) == SL_STATUS_OK) {
